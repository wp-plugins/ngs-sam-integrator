...
/*   } else if ( ( "127.0.0.1" == $samhost ) OR ( "localhost" == $samhost ) ) {
		...
		if( 0 == $throttle_daily || 0 == $throttle_number ){
			...
			if ( $daily_request_count < $throttle_daily ) {
				$recent_request_query = $wpdb->get_row( $wpdb->prepare("SELECT count(id) AS reqcount FROM $ngs_request_table WHERE host = '%s' AND t_stamp > ( NOW() - INTERVAL %s MINUTE );", $client_host, $throttle_minutes ), ARRAY_A );
...
function ngs_sam_error( $code )
{
	...
	case 850 : $request_status_message = "To request a song, you must be registered and logged into the site"; break;
	case 851 : $request_status_message = "Unregistered users are limited to $throttle_daily requests per day"; break;
